...
namespace Borntechies\Blog\Model\Import;
...
use Borntechies\Blog\Model\Config\Source\AuthorType;
use Borntechies\Blog\Model\Config\Source\Comments\Status;
use Borntechies\Blog\Model\Post;
use Borntechies\Blog\Model\Tag;

/**
 * Class WordPress
 * @package Borntechies\Blog\Model\Import
 */
class WordPress extends AbstractImport
{
    ...
    protected function _importPosts($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            foreach ($updateData as $postId => $postImage) {
                ...
                $this->_resourceConnection->getConnection()
                    ->update(
                        $this->_resourceConnection->getTableName('borntechies_blog_post'),
                        ['image' => $postImage],
                        $where
                    );
            }
        ...
        $this->_registry->register('borntechies_import_post_ids_collection', $oldPostIds);
        $this->_registry->register('borntechies_import_post_statistic', $statistics);
    ...
    protected function _importTags($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            $this->_importRelationships($data, $connection, $oldTagIds, 'borntechies_blog_post_tag', 'post_tag');
        }

        $statistics = $this->_getStatistics('tags', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('borntechies_import_tag_statistic', $statistics);
    }
    ...
    protected function _importCategories($data, $connection)
    {
        ...
        $this->_registry->register('borntechies_import_category_statistic', $statistics);
    }
    ...
    protected function _importAuthors($data, $connection)
    {
         ...
         */
        $oldPostIds = $this->_registry->registry('borntechies_import_post_ids_collection');
        $updateData = [];
        ...
        $this->_registry->register('borntechies_import_user_statistic', $statistics);
    }
        ...
        if ($isReplace && isset($sourceItems)) {
            foreach ($sourceItems as $comment) {
                ...
                if ($comment['is_imported']) {
                    ...
                    $this->_resourceConnection->getConnection()
                        ->update(
                            $this->_resourceConnection->getTableName('borntechies_blog_comment'),
                            [
            ...
            foreach ($upgradeChildData as $commentId => $commentParentId) {
                ...
                $this->_resourceConnection->getConnection()
                    ->update($this->_resourceConnection
                        ->getTableName('borntechies_blog_comment'), ['reply_id' => $commentParentId], $where);
            }
            foreach ($upgradeParentData as $commentId => $commentHasReply) {
                $where = ['comment_id = ?' => (int) $commentId];
                $this->_resourceConnection->getConnection()
                    ->update($this->_resourceConnection
                        ->getTableName('borntechies_blog_comment'), ['has_reply' => $commentHasReply], $where);
            }
            mysqli_free_result($result);
        }

        $statistics = $this->_getStatistics('comments', $this->_successCount, $this->_errorCount, $this->_hasData);
        $this->_registry->register('borntechies_import_comment_statistic', $statistics);
    }
    ...
    private function _updatePosts($post, $where)
    {
        ...
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('borntechies_blog_post_category'), $where);
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('borntechies_blog_post_tag'), $where);
    }
    ...
    private function _updateTags($tag, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update(
                $this->_resourceConnection->getTableName('borntechies_blog_tag'),
                [
                    'name'          => $tag['name'],
                    'url_key'       => $tag['url_key'],
                    'description'   => $tag['description'],
    ...
    private function _updateCategories($category, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update(
                $this->_resourceConnection->getTableName('borntechies_blog_category'),
                [
                    'name'          => $category['name'],
                    'url_key'       => $category['url_key'],
                    'description'   => $category['description'],
